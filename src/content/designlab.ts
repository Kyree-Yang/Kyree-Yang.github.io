...
Design Lab takes the intersection: the complete company design system is pre-packaged as binary artifacts on iOS and a Maven dependency on Android, so a designer gets a zero-dependency project that compiles in seconds and whose code an engineer can lift verbatim. The agentic workflow sits on top; its job is to turn the quality checklist into scripts that can refuse to let a node finish.`,
      bullets: [
        ...
        'The deliverable is a patch an engineer can apply directly',
      ],
    },
...
The stop hook is where the real leverage is. It sequences completion-check-then-bookkeeping in a single script — deliberately not two parallel hooks, because same-event hooks run concurrently and bookkeeping would happily mark a failing node complete. If the node ships a completion script, that script runs first; a non-zero exit blocks the agent from stopping and feeds the failure reason back on stderr so it keeps working. Five of the ten nodes carry such a gate, with retry ceilings so a flaky external toolchain cannot deadlock the pipeline.`,
      bullets: [
        'Gates are deterministic scripts: pattern scanners, a real compile, a translation-readiness query against the platform',
        'The localization gate (243 lines) aggregates three scanners then actually compiles the project, returning exit 2 with the error lines attached; the compile step exists for the invented-API failure mode documented in the callout under fault injection',
        'A result-required gate refuses completion if the node wrote no findings file, so a subagent cannot spend its whole budget and exit with nothing on disk',
...
The scanner classifies findings by confidence. A single-sided absolute constraint is a blocking Must; a symmetric full-width pin is a Should (visually RTL-safe, merely unidiomatic) — that distinction alone removed most of the noise. For direction icons it does a plus-or-minus three line window scan for mirroring markers across all four platform mechanisms, so already-fixed sites stop reporting. The scanner ships with a 20-case self-test to prevent regressions in the rules themselves.`,
      bullets: [
        'Blocking findings went 4 → 1 and direction-icon findings 22 → 0 in the worked scope; the single survivor is a trailing character counter that needs a human design decision, so it stays open',
...
The device step is what caught the worst failure mode, the one in the callout below: without a compile check, a node invents plausible design-system API names, and no text scanner can tell a guessed method from a real one. Compilation moved into the node's stop hook as a hard gate as a direct result.`,
      bullets: [
        ...
        `If the build fails after two retries, the node writes a blocker that says "the code does not currently compile"; it is not allowed to claim success`,
      ],
      callout: {
...

    {
      ...
      body: `Progress lives in the repository. Each screen's workflow file is the single source of truth; an aggregation script scans both platform trees, serializes every workflow file into a static data module, and a dependency-free HTML page renders it two ways — a Kanban overview that folds the DAG into linear delivery phases, and a per-screen detail view showing the full DAG with per-node status, owner, and result payload.
